...
//moq version
void moq_EnQueue(struct proc* p);
  ...
  for(;;){
	  ...
	  if(Mono){
		  ...
		  for(p=moq.front; p!=0; p=p->next){
			  ...
			  //cprintf("p->state: %s\n", p->state);
			  //release(&ptable.lock);
			  if(p->state ==ZOMBIE) moq_DeQueue(p);
...
void moq_EnQueue(struct proc* p){
	...
	//cprintf("[PID %d] enqueue to moq successfully\n", p->pid);


}
void moq_DeQueue(struct proc* p){
	if(p->ismoq !=1){
		cprintf("p is not in moq\n");
		return;
	}
	if(moq.front==0){
		cprintf("moq is null\n");
	...
	acquire(&moq.lock);
	if(p==moq.front){
		moq.front =moq.front->next;

		if(moq.front ==0) moq.rear=0;
		p->next=0;
	}
	else{
		struct proc* temp= moq.front;
		while(temp->next !=p && temp->next !=0 ){
			temp = temp->next;
		}
		temp->next = p->next;
		if(moq.rear ==p) moq.rear= temp;

		p->next =0;
	}
	release(&moq.lock);
	p->ismoq=0;
	//cprintf("[PID %d] successfully dequeued from moq\n", p->pid);

}

int setmonopoly(int pid, int password){
	//ptable에서 pid를 가진 프로세스를 찾는다.
	// moq로 이동
	// password 비교
	if(password != 2022085069) return -2;
	struct proc* p;
